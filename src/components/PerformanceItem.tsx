...
const PerformanceItem: React.FC<PerformanceItemProps> = ({ performance, index }) => {
  const [viewMode, setViewMode] = useState<'video' | 'score'>('video');
  const [activeVideoMethod, setActiveVideoMethod] = useState<keyof Performance['videoComparison']>('oursHybrid');
  const [activeScoreMethod, setActiveScoreMethod] = useState<keyof Performance['scoreComparison']>('oursHybrid');
  ...
  return (
    <div id={`performance-${performance.id}`} className="bg-white border border-gray-200 rounded-lg shadow-sm mb-16 overflow-hidden">
      {/* Title Header */}
      <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
        <h3 className="text-base font-bold text-gray-800">Example {index + 1}: {performance.title}</h3>
      </div>
          ...
          {viewMode === 'video' ? (
            <div className="max-w-4xl mx-auto">
              ...
              <div className="mt-4 text-xs text-gray-400 text-center italic">
                Live Transcription Visualization — {videoMethods.find(m => m.id === activeVideoMethod)?.label}
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              <ScoreDisplay 
                xmlUrl={performance.scoreComparison[activeScoreMethod].musicXmlUrl} 
                isPlaying={isAudioPlaying}
                onTogglePlay={() => setIsAudioPlaying(!isAudioPlaying)}
              />
              <div className="flex flex-col gap-2">
                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1 opacity-50">Score MIDI</div>
                <AudioPlayer 
